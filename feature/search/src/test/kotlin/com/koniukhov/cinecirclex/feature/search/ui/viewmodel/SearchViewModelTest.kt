package com.koniukhov.cinecirclex.feature.search.ui.viewmodel
...
import com.koniukhov.cinecirclex.feature.search.repository.FilteredMediaRepository
import com.koniukhov.cinecirclex.feature.search.repository.SearchRepository
...
class SearchViewModelTest {
    ...
    fun `init should load movie and tv series genres`() = runTest(testDispatcher) {
        ...
        newViewModel.movieGenres.test {
            ...
            Assert.assertEquals("Action", genres[28])
            Assert.assertEquals("Comedy", genres[35])
        }

        newViewModel.tvSeriesGenres.test {
            val genres = awaitItem()
            Assert.assertEquals(2, genres.size)
            Assert.assertEquals("Drama", genres[18])
            Assert.assertEquals("Sci-Fi & Fantasy", genres[10765])
        }
    ...
    @Test
    fun `selectMovieFilters should update movie filter params and reset tv series filters`() =
        runTest(testDispatcher) {
            ...
            viewModel.moviesFilterParamsState.test {
                ...
                Assert.assertEquals("popularity.desc", params?.sortBy)
                Assert.assertEquals(2024, params?.year)
                Assert.assertEquals("28,12", params?.withGenres)
            }
    ...
    @Test
    fun `selectTvSeriesFilters should update tv series filter params and reset movie filters`() =
        runTest(testDispatcher) {
            ...
            viewModel.tvSeriesFilterParamsState.test {
                val params = awaitItem()
                Assert.assertNotNull(params)
                Assert.assertEquals("vote_average.desc", params?.sortBy)
                Assert.assertEquals(2023, params?.year)
                Assert.assertEquals("18,10765", params?.withGenres)
            }
    ...
    fun `languages and countries should be initialized`() {
        Assert.assertTrue(viewModel.languages.isNotEmpty())
        Assert.assertTrue(viewModel.countries.isNotEmpty())
    }
    ...
    @Test
    fun `selectTvSeriesFilters with null should reset tv series filters`() =
        runTest(testDispatcher) {
