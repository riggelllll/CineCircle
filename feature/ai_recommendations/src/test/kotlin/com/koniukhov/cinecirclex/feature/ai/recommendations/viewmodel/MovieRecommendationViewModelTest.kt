package com.koniukhov.cinecirclex.feature.ai.recommendations.viewmodel
...
import com.koniukhov.cinecirclex.feature.ai.recommendations.MediaRepository
import com.koniukhov.cinecirclex.feature.ai.recommendations.MediaVector
...
class MovieRecommendationViewModelTest {
    ...
    @Test
    fun `generateRecommendations should calculate and return top recommendations`() =
        runTest(testDispatcher) {
            ...
            viewModel.recommendations.test {
                ...
                Assert.assertFalse(recommendations.any { it.tmdbId == 1 })
                Assert.assertFalse(recommendations.any { it.tmdbId == 2 })
    ...
    @Test
    fun `generateRecommendations should not run when userRatings is empty`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `generateRecommendationsFromDatabase should set hasNoRatings when no rated media`() =
        runTest(testDispatcher) {
            ...
            viewModel.hasNoRatings.test {
                ...
                Assert.assertTrue(hasNoRatings)
                cancelAndIgnoreRemainingEvents()
            }
